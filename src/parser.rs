...
pub fn parse_single_char(inp: &char) -> Result<ast::SKI, ast::SKIErr> {
    match inp {
        ...
        'K' => Ok(ast::SKI::K),
        'S' => Ok(ast::SKI::S),
        _ => Err(ast::SKIErr::ParseError(String::from("not a SKI primitive"))),
    }
}
...
pub fn maybe_parse_single_char(inp: &Option<char>) -> Result<ast::SKI, ast::SKIErr> {
    match inp {
        None => Err(ast::SKIErr::ParseError(String::from("empty input"))),
...
pub fn match_all_parens(
    open_parens: &mut Vec<usize>,
    close_parens: &mut Vec<usize>,
